...
define('BOT_TOKEN', '264536652:AAGGRcpxFTs2M_SxcelKcs5jb-ddY8_EFaI');
...
به ربات ما امتیاز بدید  👈        [⭐️payam resan saz⭐️](https://telegram.me/storebot?start=payam_resan_saz_bot)
...
";
      ...
      apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "<b>Payam resan saz</b>
<b>ver. 3.0</b>
...
Copy Right 2016©","parse_mode"=>"html"));
    ...
    }else if ($matches[0] == "/sendtoall"&& $chat_id == 200546170) {
		...
		for($y=0;$y<count($banid);$y++)
      {
        apiRequest("sendMessage", array('chat_id' => $banid[$y], "text" => $sendtotext));
          ...
          apiRequest("sendMessage", array('chat_id' => 200546170, "text" => "Sent To ".count($banid)." Members"));
...
🗣پیام ارسال شد");
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[1].'/setwebhook?url=https://am-amirab247.rhcloud.com/'.$chat_id.'/pvresan.php');
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "🚀 ربات شما با مـوفقیت آپدیت شد ♻️"));


    }
    }
    else if ($matches[0] != "/update"&& $matches[1]==""&&$chat_id != 200546170) {
      if (strpos($text, ":")) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$text.'/setwebhook?url=https://am-amirab247.rhcloud.com/'.$chat_id.'/pvresan.php');
...
<b> by TeleSpeed Tm </b>";
    ...
    $bot_url    = "https://api.telegram.org/bot264536652:AAGGRcpxFTs2M_SxcelKcs5jb-ddY8_EFaI/"; 
...
🚀 @veryg0odebot"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == 200546170) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[0].'/setwebhook?url=https://am-amirab247.rhcloud.com/'.$id.'/pvresan.php');
    $unstalled = "ربات شما با موفقیت نصب شده است🚀 
برای ورود به ربات خود کلیک کنید 👇😃
به ربات ما امتیاز بدید  👇
 https://telegram.me/storebot?start=payam_resan_saz_bot
